...
def displayReport(tasks,indent):
    ...
    for task in tasks:
        if 'subtasks' in task:
            print(indent * sep + str(task.get("id")) + "." + task.get("description") + " (" + task.get(
                "assigned_to") + ") ")
            indent = indent + 1
            displayReport(task.get("subtasks"), indent)
        ...
        if found:
            ...
            for task in tasks:
                if task.get('id') == task_id:
                    if "subtasks" in task:
                        task["subtasks"].insert(0, {'id': task_id+1, 'description': description,
                                                    'assigned_to': assigned_to, 'time_estimate': time_estimate})
                    else:
                        task["subtasks"] = [{'id': task_id+1 , 'description': description, 'assigned_to': assigned_to,
...
def assign_task(tasks,task_id, new_member):
    for task in tasks:
        if task.get('id') == task_id:
            ...
            print("Task " + task.get("description") + " assigned to " + new_member)
        elif 'subtasks' in task:
...
def complete_task_recursive(tasks, task_id):
    for task in tasks:
        if task.get('id') == task_id:
            task['completed'] = True
            print("Task \'" + task.get("description") +"\' marked as completed.")
            if 'subtasks' in task:
...
def generate_report_recursive(tasks, indent):
    ...
    for task in tasks:
        ...
        status = 'Pending' if remainingHours == 0 else 'Completed'
    ...
    for task in tasks:
        ...
        if 'subtasks' in task:
            print(indent * sep + str(task.get("id")) + "." + task.get("description") + " (" + task.get("assigned_to") +\
            ") " + sep + " Estimated time to finish: " + str(calculate_time_recursive(task,True)) + " out of " +\
            str(calculate_time_recursive(task,False)) , end="")
            print(" Completed" if calculate_time_recursive(task,True)==0 else " Pending")


            generate_report_recursive(task.get("subtasks"), indent + 1)

        else:
            print(indent * sep + str(task.get("id")) + "." + task.get("description") + " (" + task.get("assigned_to") +\
            ") " + sep + " Estimated time to finish: " + str(calculate_time_recursive(task,True)) + " out of " +\
            str(calculate_time_recursive(task,False)) +" "+ isComplete)
    """
